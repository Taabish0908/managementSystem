...
class adminController{
    ...
    async register(req,res){
        try {
            ...
            let isMailAlreadyExist = await adminModel.findOne({email:data.email})
            ...
            let isPhoneAlreadyExist  = await adminModel.findOne({phone:data.phone})
            ...
            const createdUser = await adminModel.create(userData);
            return res.status(200).send({
                status:true,
                message:'user creataed successfully',data:createdUser
            })
    ...
    async login(req,res){
        ...
        try {
            ...
            let foundUser =  await adminModel.findOne({email:data.email,phone:data.phone})
            if(!foundUser){
                return res.status(400).send({
                    staus:false,
